<?php

namespace DataWarehouse\Query\SUPREMM;

class JobTimeseriesTest extends \PHPUnit_Framework_TestCase
{
    public function __construct() {
        $v4json =  <<<EOF
{
    "_id" : "4454065-1441406017",
    "cpuuser" : {
        "hosts" : {
            "0" : {
                "all" : [ 
                    77.1722482882529, 
                    97.9114425210127, 
                    99.1613659762888, 
                    99.2024109481888, 
                    99.5197982572934, 
                    99.4205887385454, 
                    99.4880273700893, 
                    99.2921719102813, 
                    99.4953957894563, 
                    99.3174315022717
                ],
                "names" : {
                    "1" : "cpu1",
                    "0" : "cpu0",
                    "3" : "cpu3",
                    "2" : "cpu2",
                    "5" : "cpu5",
                    "4" : "cpu4",
                    "7" : "cpu7",
                    "6" : "cpu6"
                },
                "dev" : {
                    "1" : [ 
                        68.4915895494676, 
                        86.6657306454345, 
                        96.0989009990808, 
                        96.894215414157, 
                        99.6156466455484, 
                        99.5622491197838, 
                        99.600527966437, 
                        99.5129600416531, 
                        99.5537410831253, 
                        99.4007654773533
                    ],
                    "0" : [ 
                        80.7160378108282, 
                        99.332260508998, 
                        99.532195068767, 
                        99.4273975164879, 
                        99.5489695058927, 
                        99.5289172653747, 
                        99.3671933962345, 
                        97.7133284802836, 
                        98.3868352097464, 
                        98.0674218760474
                    ],
                    "3" : [ 
                        81.9298128155024, 
                        99.3655934823231, 
                        99.532195068767, 
                        99.527391546843, 
                        99.4822923662371, 
                        98.4289660698766, 
                        99.567194456408, 
                        99.5796130624445, 
                        99.5204009153144, 
                        99.5674334275165
                    ],
                    "2" : [ 
                        80.3692449523499, 
                        99.4989253756238, 
                        99.4988621166342, 
                        99.527391546843, 
                        99.5823080757205, 
                        99.5622491197838, 
                        99.6338614764659, 
                        99.6795925936317, 
                        99.8204624256119, 
                        99.6674341976145
                    ],
                    "5" : [ 
                        67.0177199009347, 
                        99.5989242955993, 
                        99.5655280208999, 
                        99.5607228902948, 
                        99.5823080757205, 
                        99.4622535565566, 
                        99.0671918059741, 
                        99.0463888961128, 
                        99.4537205796928, 
                        99.5340998374839
                    ],
                    "4" : [ 
                        80.976132454687, 
                        99.3655934823231, 
                        99.3988632602355, 
                        99.294072142681, 
                        99.6156466455484, 
                        99.1622668668753, 
                        99.2005258460898, 
                        99.3463274896744, 
                        99.5870812509361, 
                        98.867428036831
                    ],
                    "7" : [ 
                        78.3751860160996, 
                        99.8322551088755, 
                        99.9321904943615, 
                        99.7273796075534, 
                        99.2489223774423, 
                        99.762240246238, 
                        99.600527966437, 
                        99.646266083236, 
                        99.7204419221794, 
                        99.7007677876471
                    ],
                    "6" : [ 
                        79.5022628061541, 
                        99.6322572689245, 
                        99.7321927815643, 
                        99.6607169206499, 
                        99.4822923662371, 
                        99.8955676638741, 
                        99.8671960466684, 
                        99.8128986352146, 
                        99.9204829290443, 
                        99.7341013776798
                    ]
                }
            }
        },
        "times" : [ 
            1441405309.25135, 
            1441405339.25167, 
            1441405369.25202, 
            1441405399.25381, 
            1441405429.2491, 
            1441405459.25043, 
            1441405489.25027, 
            1441405519.25641, 
            1441405549.25026, 
            1441405579.25003
        ]
    },
    "version" : 4,
    "hosts" : {
        "0" : "cpn-d09-25-02.cbls.ccr.buffalo.edu"
    },
    "schema": {
        "metrics": {
            "cpuuser" : {
                "units" : "CPU %",
                "description" : "CPU User",
                "help" : "The average percentage of time spent in CPU user mode. The average is computed over each time interval."
            }
        }
    }
}
EOF;
        $this->v4doc = json_decode($v4json, true);
    }

    public function testGetData() {

        $timeseries = new \DataWarehouse\Query\SUPREMM\JobTimeseries($this->v4doc);

        $timeseries->get('cpuuser', null, null);
        
    }
}
